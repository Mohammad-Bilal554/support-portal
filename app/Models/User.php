...
class User extends Model
{
    ...
    public static function getAllEmployees(): array
    {
        return static::db()->fetchAll(
            "SELECT * FROM users WHERE role IN ('super_admin','employee') AND is_active = 1 ORDER BY first_name"
        );
    }
    ...
    public static function avatarUrl(array $user): string
    {
        ...
        return 'https://ui-avatars.com/api/?name=' . urlencode(static::fullName($user))
    ...
    {
        $key = 'login_attempts_' . md5($email . $ip);
        Database::getInstance()->delete('settings', ['key_name' => $key]);
    }
