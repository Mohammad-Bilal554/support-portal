...
class View {
    ...
    public function render(): string {
        // Views that self-include their layout (auth views, dashboard views)
        // will use ob_start() internally and call require layout.
        // We render the view file and capture whatever it outputs.
        $output = $this->renderFile($this->resolvePath($this->viewPath), $this->data);

        // If explicit layout set (alternative pattern), wrap output
        if ($this->layout && !empty($output)) {
            $this->data['content'] = $output;
            return $this->renderFile($this->resolvePath($this->layout), $this->data);
        }
    ...
    public function csrf(): string {
        return '<input type="hidden" name="_csrf_token" value="' . htmlspecialchars(Csrf::getToken(), ENT_QUOTES, 'UTF-8') . '">';
    }
